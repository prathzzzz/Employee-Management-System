...
namespace NestleECS_final
{
    public partial class salaryControl : UserControl
    {
        ...
        private void buttonSearch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string query = "select emp.id,emp.name, emp.designation,emp.department,emp.doj, emp.remarks,  sal.salary_type, sal.salary_amount from employee.employee emp INNER JOIN employee.salary sal ON sal.employee_id = emp.id where emp.id = " + g_id + " ";
                ...
                if (dt.Rows.Count == 1)
                {
                    ...
                    foreach (DataRow item in dt.Rows)
                    {
                        ...
                        salaryBox.Text = item[7].ToString();
                    }
                    makeReadOnly();
                }
                else
        ...
        private void button_save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string query_update = "update  employee.salary set salary_type = '" + this.comboBox1.Text + "' ,salary_amount = '" + this.salaryBox.Text + "'  where employee_id = " + g_id + " ";
                g_id = 0;
